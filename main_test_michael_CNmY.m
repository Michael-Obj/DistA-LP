...
for i = 1:num_locations
    % Select the CARDINALITY_N - 1 nearest neighbors for the current location
...
for numIntervals = 1:12
    ...
    % [quantized_matrices, best_pi] = align_user_matrix_A2(aligned_raw_distance_matrices);
...
avg_val = mean(average_posterior_leakage);
...
xlabel('abs(log(Posterior Leakage))');
ylabel('Cumulative Probability');
title('CDF of Posterior Leakage with Overall Mean');
grid on;
