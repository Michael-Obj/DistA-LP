    ...
    methods
                ...
                for j = 1:1:user.LR_loc_size
                    if i ~= j && user.distance_matrix_LR(i, j) <= user.neighbor_threshold
                        for k = 1:1:size(user.obf_loc_ID, 2)
                            ...
                            this.geoI_matrix(idx, (j-1)*size(user.obf_loc_ID, 2) + k) = -exp(user.epsilon*user.distance_matrix_LR(i,j)); 
                            ...
                            Qmask   = this.Q_matrix_obf;                            % 1×(LR*|obf|)
                            DLR2OBF = reshape(user.distance_matrix_LR2obf', 1, []); % 1×(LR*|obf|)
                            
                            this.geoI_matrix_B(idx, :) = this.geoI_matrix(idx, :) .* (Qmask .* exp(user.epsilon * DLR2OBF));
                            idx = idx + 1;
                        end
                    end
                end
        ...
        function this = unit_matrix_cal(this, user)
            ...
            for i = 1:1:user.LR_loc_size
                ...
                this.Unit_matrix_B(i,:) = this.Unit_matrix(i,:) .* (Qmask .* exp(user.epsilon * DLR2OBF));
            end   
        ...
        function [this, user] = calculate(this, user, m, master_program, env_parameters)
            ...
            for i = 1:1:user.LR_loc_size
                for k = 1:1:size(user.obf_loc_ID, 2)
                    y_transform((i-1)*size(user.obf_loc_ID, 2) + k, k) = exp(-user.epsilon*user.distance_matrix_LR2obf(i,k)); 
                end
            end
            ...
            'Display','none','MaxIterations',1e5, ...
