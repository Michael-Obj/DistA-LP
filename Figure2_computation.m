...
S = load('Bendersdecomposition_PETS_Qiu_SP/rome_location_data_2000_nodes/location_data_sample_1/location_data_r1_user1.mat', ...
...
R = load('Bendersdecomposition_PETS_Qiu_SP/rome_location_data_2000_nodes/location_data_sample_1/location_data_r1_user1.mat','LR_ID');
...
best_factors = cell(num_locations, 1);
...
%     [best_pi{i}, best_factors{i}, GF1{i}, GF2{i}, GF3{i}] = reorder_fit_lowrank_svd(A1, A2, A3, rank_r, 1.0, 1.0);
% 
%     F1_norm{i} = diag(best_factors{i}.S1)';    % 1×r
%     F2_norm{i} = diag(best_factors{i}.S2)';    % 1×r
%     F3_norm{i} = diag(best_factors{i}.S3)';    % 1×r
% 
%     % Utility loss
%     B3 = GF3{i};
% 
...
for i = 1:num_locations   
    ...
    for sample_idx = 1:num_samples
        ...
        % EPSILON = sample_idx; 
        noisy_distance_sample_1 = add_noise_to_distance_matrix(original_distance_matrices{i}, EPSILON);  
        ...
        [posterior_prob_sample_1{sample_idx_}, PL_matrix_1{sample_idx_}, PL_max_1(i, sample_idx)] = compute_log_posterior(original_distance_matrices, noisy_distance_sample_1, distance_matrix, num_locations, B_VALUE, CARDINALITY_N); 
    ...
    for sample_idx = 1:num_samples
        % Add noise to the distance matrix to the location i to generate a new noisy sample
        % EPSILON = sample_idx; 
        noisy_distance_sample_2 = add_noise_to_distance_matrix(obfuscated_distance_matrices{i}, EPSILON);  
        ...
        [posterior_prob_sample_2{sample_idx__}, PL_matrix_2{sample_idx__}, PL_max_2(i, sample_idx)] = compute_log_posterior(obfuscated_distance_matrices, noisy_distance_sample_1, distance_matrix, num_locations, B_VALUE, CARDINALITY_N);    
    ...
    for sample_idx = 1:num_samples
        % Add noise to the distance matrix to the location i to generate a new noisy sample
        % EPSILON = sample_idx; 
        noisy_distance_sample = add_noise_to_distance_matrix(cost_coefficient_matrices{i}, EPSILON);  

        % Compute the posterior probability for this noisy distance sample
        sample_idx___ = (i-1)*num_samples + sample_idx; 
        [posterior_prob_sample_3{sample_idx___}, PL_matrix_3{sample_idx___}, PL_max_3(i, sample_idx)] = compute_log_posterior(cost_coefficient_matrices, noisy_distance_sample_1, distance_matrix, num_locations, B_VALUE, CARDINALITY_N); 
    ...
    % Store the privacy loss for this location
    % privacy_loss(i, :) = privacy_loss_samples';

    % % Print all privacy losses for this location
...
fprintf('SVD Fit time: %.2f seconds\n', time_svd_fit );
...
% 
% hold off;
% xlabel('abs(log(Privacy Leakage1))');
% ylabel('Cumulative Probability 1');
% title('CDF of Privacy Leakage 1 with Mean and Selected CDF Thresholds');
% grid on;
% % ===================================================================================
...
% 
% hold off;
% xlabel('abs(log(Privacy Leakage2))');
% ylabel('Cumulative Probability 2');
% title('CDF of Privacy Leakage 2 with Mean and Selected CDF Thresholds');
% grid on;
% % ===================================================================================
...
% 
% hold off;
% xlabel('abs(log(Privacy Leakage3))');
% ylabel('Cumulative Probability 3');
% title('CDF of Privacy Leakage 3 with Mean and Selected CDF Thresholds');
% grid on;
...
%% --- Percent of privacy loss exceeding epsilon (global, all entries) ---
threshold = EPSILON;  % your privacy budget

pct_over_eps_1 = 100 * mean(pooled_1_vec > threshold);
pct_over_eps_2 = 100 * mean(pooled_2_vec > threshold);
pct_over_eps_3 = 100 * mean(pooled_3_vec > threshold);

fprintf('\n[Global %% > epsilon]\n');
fprintf('PL1: %.4f%% of entries > EPSILON (%.4f)\n', pct_over_eps_1, threshold);
fprintf('PL2: %.4f%% of entries > EPSILON (%.4f)\n', pct_over_eps_2, threshold);
fprintf('PL3: %.4f%% of entries > EPSILON (%.4f)\n', pct_over_eps_3, threshold);

%% --- Percent of locations whose supremum exceeds epsilon (per-location) ---
loc_over_eps_1 = 100 * mean(privacy_loss_supremum_1 > threshold);
loc_over_eps_2 = 100 * mean(privacy_loss_supremum_2 > threshold);
loc_over_eps_3 = 100 * mean(privacy_loss_supremum_3 > threshold);

fprintf('\n[Per-location (supremum) %% > epsilon]\n');
fprintf('PL1: %.4f%% of locations have sup(abs(log PL)) > EPSILON (%.4f)\n', loc_over_eps_1, threshold);
fprintf('PL2: %.4f%% of locations have sup(abs(log PL)) > EPSILON (%.4f)\n', loc_over_eps_2, threshold);
fprintf('PL3: %.4f%% of locations have sup(abs(log PL)) > EPSILON (%.4f)\n', loc_over_eps_3, threshold);

%% --- Optional: mark epsilon on existing PDF plots ---
figure; 
[f, xi] = ksdensity(pooled_1_vec);
XY = [xi(:), f(:)];
writematrix(XY, 'PL1_pdf_xy.csv');              % CSV
save('PL1_pdf_xy.mat', 'xi', 'f', 'XY');        % MAT
plot(xi, f, 'LineWidth', 2); hold on;
xline(threshold, '--', sprintf('\\epsilon = %.4f', threshold), 'LineWidth', 1.2);
xlabel('abs(log(Privacy Loss))'); ylabel('Density');
title('PDF of Privacy Loss 1 with \\epsilon'); grid on;

figure; 
[f2, xi2] = ksdensity(pooled_2_vec);
writematrix([xi2(:), f2(:)], 'PL2_pdf_xy.csv');
save('PL2_pdf_xy.mat', 'xi2', 'f2');
plot(xi2, f2, 'LineWidth', 2); hold on;
xline(threshold, '--', sprintf('\\epsilon = %.4f', threshold), 'LineWidth', 1.2);
xlabel('abs(log(Privacy Loss))'); ylabel('Density');
title('PDF of Privacy Loss 2 with \\epsilon'); grid on;

figure; 
[f3, xi3] = ksdensity(pooled_3_vec);
writematrix([xi3(:), f3(:)], 'PL3_pdf_xy.csv');
save('PL3_pdf_xy.mat', 'xi3', 'f3');
plot(xi3, f3, 'LineWidth', 2); hold on;
xline(threshold, '--', sprintf('\\epsilon = %.4f', threshold), 'LineWidth', 1.2);
xlabel('abs(log(Privacy Loss))'); ylabel('Density');
title('PDF of Privacy Loss 3 with \\epsilon'); grid on;
