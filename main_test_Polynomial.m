...
for i = 1:num_locations
    ...
    [best_pi{i}, best_coeffs{i}, GF1{i}, GF2{i}, GF3{i}, stats_i] = ...
    reorder_fit_polynomials_(A1, A2, A3, deg, 1.0, 1.0);
...
t_total = t_total_fit(:);
t_swap  = t_swap_sum(:);
t_fit   = t_fmin_sum(:);   % includes init + per-iter
...
for i = 1:num_locations
    t_fit_init(i) = stats_store{i}.time_fit_init;
    t_fit_loop(i) = sum(stats_store{i}.time_fit);
    swap_evals_total(i)   = sum(stats_store{i}.swap_evals);
    swap_accepts_total(i) = sum(stats_store{i}.swap_accepts);
end

ResultsTime = table( (1:num_locations)', iter_count(:), converged(:), stop_reason(:), ...
    ...
    'TimeTotal_s','TimePermSwap_s','TimeFitInit_s','TimeFitLoop_s','TimeFitTotal_s', ...
    'PermFrac_pct','FitFrac_pct','SwapEvals','SwapAccepts','FinalLoss'});

disp(ResultsTime);
writetable(ResultsTime, 'time_efficiency_polynomial.csv');


fprintf('\n==== Alternating Method: Time-Efficiency ====\n');
fprintf('Locations evaluated: %d\n', num_locations);
fprintf('Convergence rate: %.1f%%\n', 100*mean(converged));

fprintf('\n-- Total runtime per location (seconds) --\n');
...
% ===== Create zoomed inset =====
% ax_inset = [0.47 0.33 0.4 0.35];       % 10 iterations
ax_inset = [0.42 0.27 0.45 0.41];        % [left bottom width height] [0.58 0.35 0.3 0.35];  
...
% ===== Create zoomed inset =====
% ax_inset = [0.7 0.2 0.2 0.15];             % 10 iterations  
ax_inset = [0.68 0.18 0.22 0.17];          % [left bottom width height] [0.58 0.35 0.3 0.35];  
axes('Position', ax_inset);  
...
for i = 1:num_locations   
    ...
    for sample_idx = 1:num_samples
        ...
        PL_matrix_2{sample_idx__} = [];                                             % free the per-sample matrix once saved:
    end
...
fprintf('Polynomial Fit time: %.2f seconds\n', time_polynomial_fit );
