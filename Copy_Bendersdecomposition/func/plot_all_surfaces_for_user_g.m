function plot_all_surfaces_for_user_g(u, outdir)
...
% outdir : folder to save PNGs (created if missing)
    ...
    if nargin < 2 || isempty(outdir), outdir = "london_fig_surfaces_6000/fig_surfaces_g"; end
    if ~exist(outdir, 'dir'), mkdir(outdir); end

    % 1) distance_matrix_LR
    plot_matrix_pair_surf( ...
        u.distance_matrix_LR, ...
        u.distance_matrix_LR_recovered, ...
        'distance\_matrix\_LR (original)', ...
        'distance\_matrix\_LR\_recovered', ...
        fullfile(outdir, sprintf('surf_LR_user%02d.png', u.loc_ID)) );

    % 2) distance_matrix_LR2obf
    plot_matrix_pair_surf( ...
        u.distance_matrix_LR2obf, ...
        u.distance_matrix_LR2obf_recovered, ...
        'distance\_matrix\_LR2obf (original)', ...
        'distance\_matrix\_LR2obf\_recovered', ...
        fullfile(outdir, sprintf('surf_LR2obf_user%02d.png', u.loc_ID)) );

    % 3) cost_matrix_RL
    plot_matrix_pair_surf( ...
        u.cost_matrix_RL, ...
        u.cost_matrix_RL_recovered, ...
        'cost\_matrix\_RL (original)', ...
        'cost\_matrix\_RL\_recovered', ...
        fullfile(outdir, sprintf('surf_cost_RL_user%02d.png', u.loc_ID)) );
end
