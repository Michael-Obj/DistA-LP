...
function val = gaussian2d(x, y, cx, cy, sx, sy, A, b)
    smin = 1e-2; smax = 10*max(size(x));    % keep widths reasonable
    sx = min(max(abs(sx), smin), smax);
    sy = min(max(abs(sy), smin), smax);
    A  = max(A, 0);                          % nonnegative amplitude
    b  = max(b, 0);                          % nonnegative offset
