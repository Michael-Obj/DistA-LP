% using real data, worked with original locations only.
...
function main_function() 
    ...
    [col_osmid_selected, original_longitude, original_latitude] = node_in_target(col_longitude, col_latitude, col_osmid, TARGET_LON_MAX, TARGET_LON_MIN, TARGET_LAT_MAX, TARGET_LAT_MIN);

    % Debug: Print the number of selected nodes
    fprintf("Number of selected nodes: %d\n", length(col_osmid_selected));
    disp('Col_Osmid_Selected:');
    disp(col_osmid_selected);

    disp('Original Longitude');
    disp(original_longitude);

    disp('Original Latitude');
    disp(original_latitude);

    % The following plot is just for testing 
    figure;
    plot(col_longitude, col_latitude, 'o'); 
    hold on; 
    plot(original_longitude, original_latitude, 'ro', 'MarkerFaceColor', 'r'); % Highlight selected nodes
    grid on;
    hold off;

    % Compute pairwise Haversine distances
    raw_distance_matrix = compute_raw_distance_matrix(original_longitude, original_latitude);

    disp('Raw Distance Matrix (Original vs Original):');
    ...
    posterior_probabilities_main = compute_posterior_probabilities(original_longitude, original_latitude, raw_distance_matrix, noisy_distance_matrix);
    ...
    for sample_idx = 1:num_samples
        ...
        posterior_probabilities_sample = compute_posterior_probabilities(original_longitude, original_latitude, raw_distance_matrix, noisy_distance_sample);
...
% Function to select nodes within the target region
function [col_osmid_selected, original_longitude, original_latitude] = node_in_target(col_longitude, col_latitude, col_osmid, TARGET_LON_MAX, TARGET_LON_MIN, TARGET_LAT_MAX, TARGET_LAT_MIN)
    % Find indexes of nodes within the target region
    ...
    % Select only the first thirty values if available
    num_selected = min(30, length(col_osmid_selected));
    col_osmid_selected = col_osmid_selected(1:num_selected);
    original_longitude = col_longitude_selected(1:num_selected);
    original_latitude = col_latitude_selected(1:num_selected);
end

function raw_distance_matrix = compute_raw_distance_matrix(original_longitude, original_latitude)
    num_original = length(original_longitude);
    raw_distance_matrix = zeros(num_original, num_original);  % Now comparing original locations with themselves
    for i = 1:num_original
        for j = 1:num_original
            raw_distance_matrix(i, j) = haversine_distance(original_latitude(i), original_longitude(i), original_latitude(j), original_longitude(j));
        end
    end
end
...
function noisy_distance_matrix = add_noise_to_distance_matrix(distance_matrix, sensitivity, epsilon)
    ...
    noise_matrix = min(noise_matrix, distance_matrix - 0.3);
...
function posterior_probabilities = compute_posterior_probabilities(original_longitude, original_latitude, raw_distance_matrix, noisy_distance_matrix)
    num_original = length(original_longitude);
    P_A = ones(num_original, 1) / num_original;  % Prior probabilities for original locations
    cardinality_N = num_original * num_original;  % Adjusted for original locations only
    b = 0.5;
    P_B_given_A = zeros(num_original, num_original);  % Using original locations for both A and B
    for i = 1:num_original
        for j = 1:num_original
            dist = abs(noisy_distance_matrix(i, j) - raw_distance_matrix(i, j));  % Compare original vs. original
            P_B_given_A(i, j) = (1 / (2 * b))^cardinality_N * exp(-dist / b);
        end
    end
    P_B = sum(P_B_given_A .* P_A, 1);
    posterior_probabilities = (P_B_given_A .* P_A) ./ P_B;  % Posterior probabilities
end

function leakage_value = compute_posterior_leakage_value(posterior_probabilities)
    % Find the maximum log of posterior probability ratio for leakage
    num_original = size(posterior_probabilities, 1);
    PL = zeros(num_original, num_original);  % A matrix to store leakage values

    % Iterate over pairs of original locations
    for n = 1:num_original
        for m = 1:num_original
            if n ~= m
                posterior_ratio = posterior_probabilities(n, :) ./ posterior_probabilities(m, :);  % Compare the posterior probabilities for different locations
                prior_ratio = 1;   % Since they are the same

                log_term = log(posterior_ratio / prior_ratio); % Take log of the ratios
                PL(n, m) = max(abs(log_term));  % Store the maximum absolute log term for leakage
            end
        end
    end

    leakage_value = max(PL(:));  % The maximum value in the leakage matrix
end