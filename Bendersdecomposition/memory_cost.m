...
cost             = nan(3, R);   
cost_test        = nan(3, R);   
computation_time = nan(3, R);  
nr_violations    = nan(3, R);  
violation_mag    = nan(3, R);  

% ---------------------------------------------------------
% Server-only memory-cost metrics
% This measures only:
%   server.geo_obfuscation_initialization(...)
%   server.geo_obfuscation_generator(...)
% ---------------------------------------------------------
server_mem_before_bytes = nan(3, R);
server_mem_after_bytes  = nan(3, R);
server_mem_delta_bytes  = nan(3, R);
server_mem_delta_kb     = nan(3, R);
server_mem_delta_mb     = nan(3, R);

server_runtime_sec      = nan(3, R);
server_nr_iterations    = nan(3, R);

server_opt_success      = false(3, R);
server_error_message    = cell(3, R);
...
% budget_assigned_rome_svd = [ 1.62388+0.845337+1.66844, 1.8756+1.27814+1.67027, 2.02693+1.14313+1.24099, 2.48513+2.1237+1.90402;
%                               1.13521+0.583166+1.17936, 1.31269+0.890921+1.16058, 1.40984+0.768284+0.840288, 1.74501+1.4703+1.28759;
%                               0.647849+0.343846+0.663307, 0.750463+0.506235+0.655254, 0.79739+0.443967+0.453896, 1.00592+0.824146+0.705942];
...
for ep_idx = 1:1:3   
    ...
    % for r = 1:R
        ...
        try
            ...
            [mem_before, ~] = memory;
            server_mem_before_bytes(ep_idx, r) = mem_before.MemUsedMATLAB;
        
            t_server = tic;
        
            server = server.geo_obfuscation_initialization(user, env_parameters);        
        
            [server, user, nr_iterations, cost(ep_idx, r), cost_test(ep_idx, r)] = ...
                server.geo_obfuscation_generator(user, env_parameters);
        
            server_runtime_sec(ep_idx, r) = toc(t_server);
            computation_time(ep_idx, r)   = server_runtime_sec(ep_idx, r);
        
            [mem_after, ~] = memory;
            server_mem_after_bytes(ep_idx, r) = mem_after.MemUsedMATLAB;
        
            delta_bytes = server_mem_after_bytes(ep_idx, r) - server_mem_before_bytes(ep_idx, r);
        
            server_mem_delta_bytes(ep_idx, r) = delta_bytes;
            server_mem_delta_kb(ep_idx, r)    = delta_bytes / 1024;
            server_mem_delta_mb(ep_idx, r)    = delta_bytes / (1024^2);
        
            server_nr_iterations(ep_idx, r) = nr_iterations;
            server_opt_success(ep_idx, r)   = true;
        
            cost(ep_idx, r)      = cost(ep_idx, r) / 20;
            cost_test(ep_idx, r) = cost_test(ep_idx, r) / 20;
        
            [nr_violations(ep_idx, r), violation_mag(ep_idx, r)] = ...
                GeoInd_violation_cnt(user, env_parameters);
        
            fprintf('SERVER SUCCESS: epsilon=%.2f, region=%d, runtime=%.4f sec, memory delta=%.4f MB\n', ...
                EPSILON, r, server_runtime_sec(ep_idx, r), server_mem_delta_mb(ep_idx, r));
        
        catch ME
        
            server_opt_success(ep_idx, r) = false;
            server_error_message{ep_idx, r} = getReport(ME, 'extended', 'hyperlinks', 'off');
        
            fprintf('SERVER FAILED: epsilon=%.2f, region=%d, error=%s\n', ...
                EPSILON, r, ME.message);
        
            % Skip the rest of this region/epsilon combination
            continue;
...
% ---------------------------------------------------------
% Save server-only memory metrics
% ---------------------------------------------------------
save("server_memory_cost.mat", ...
    "server_mem_before_bytes", ...
    "server_mem_after_bytes", ...
    "server_mem_delta_bytes", ...
    "server_mem_delta_kb", ...
    "server_mem_delta_mb", ...
    "server_runtime_sec", ...
    "server_nr_iterations", ...
    "server_opt_success", ...
    "server_error_message");
...
% ---------------------------------------------------------
% Build detailed server-memory table
% ---------------------------------------------------------
rows = [];
...
for ep = 1:3
    for rr = 1:R
        ...
        rows = [rows; ...
            ...
            epsilon_value(ep), ...
            rr, ...
            server_opt_success(ep, rr), ...
            server_runtime_sec(ep, rr), ...
            server_nr_iterations(ep, rr), ...
            server_mem_before_bytes(ep, rr), ...
            server_mem_after_bytes(ep, rr), ...
            server_mem_delta_bytes(ep, rr), ...
            server_mem_delta_kb(ep, rr), ...
            server_mem_delta_mb(ep, rr)];
    end
end

ServerMemoryTable = array2table(rows, ...
    'VariableNames', { ...
    'epsilon_index', ...
    'epsilon_total', ...
    'region_id', ...
    ...
    'mem_delta_kb', ...
    'mem_delta_mb'});
ServerMemoryTable.mem_delta_mb_nonnegative = ServerMemoryTable.mem_delta_mb;
ServerMemoryTable.mem_delta_mb_nonnegative(ServerMemoryTable.mem_delta_mb_nonnegative < 0) = NaN;

writetable(ServerMemoryTable, ...
    fullfile(resultsDir, 'server_only_memory_cost_detailed.csv'));

disp('Server-only memory-cost detailed table:');
disp(ServerMemoryTable);

% ---------------------------------------------------------
% Build summary table by epsilon
% ---------------------------------------------------------
summaryRows = [];

for ep = 1:3

    idx = ServerMemoryTable.epsilon_index == ep & ...
          ServerMemoryTable.success == 1 & ...
          isfinite(ServerMemoryTable.mem_delta_mb);

    T = ServerMemoryTable(idx, :);
    ...
    if height(T) > 0
        ...
        summaryRows = [summaryRows; ...
            ...
            epsilon_value(ep), ...
            height(T), ...
            mean(T.server_runtime_sec, 'omitnan'), ...
            std(T.server_runtime_sec, 0, 'omitnan'), ...
            min(T.server_runtime_sec), ...
            median(T.server_runtime_sec), ...
            max(T.server_runtime_sec), ...
            mean(T.mem_delta_mb_nonnegative, 'omitnan'), ...
            std(T.mem_delta_mb_nonnegative, 0, 'omitnan'), ...
            mean(T.mem_delta_mb, 'omitnan'), ...
            std(T.mem_delta_mb, 0, 'omitnan'), ...
            min(T.mem_delta_mb), ...
            median(T.mem_delta_mb), ...
            max(T.mem_delta_mb), ...
            mean(T.nr_iterations, 'omitnan'), ...
            std(T.nr_iterations, 0, 'omitnan')];

    else

        summaryRows = [summaryRows; ...
            ep, epsilon_value(ep), 0, ...
            NaN, NaN, NaN, NaN, NaN, ...
            NaN, NaN, NaN, NaN, NaN, ...
            NaN, NaN, NaN, NaN];
...
ServerMemorySummary = array2table(summaryRows, ...
    'VariableNames', { ...
    'epsilon_index', ...
    'epsilon_total', ...
    'successful_regions', ...
    'mean_server_runtime_sec', ...
    'std_server_runtime_sec', ...
    'min_server_runtime_sec', ...
    'median_server_runtime_sec', ...
    'max_server_runtime_sec', ...
    'mean_server_mem_delta_mb_nonnegative', ...
    'std_server_mem_delta_mb_nonnegative', ...
    'mean_server_mem_delta_mb', ...
    'std_server_mem_delta_mb', ...
    'min_server_mem_delta_mb', ...
    'median_server_mem_delta_mb', ...
    'max_server_mem_delta_mb', ...
    'mean_nr_iterations', ...
    'std_nr_iterations'});

writetable(ServerMemorySummary, ...
    fullfile(resultsDir, 'server_only_memory_cost_summary.csv'));

disp('Server-only memory-cost summary:');
disp(ServerMemorySummary);
